...
void odomMsgCallback(const nav_msgs::Odometry &nodom){
	omutex.lock();{
	g_odom = nodom;
	} mutex.unlock();
}
...
void convertOdom2XYZRPY(nav_msgs::Odometry &odom, Vec3d &xyz, Vec3d &rpy) { // 이동 저장
...
int main(int argc, char **argv)
{
	...
	converOdom2XYZRPY(odom, xyz, rpy);
	transform(laserScanXY, xyz[0], xyz[1], rpy[1]);
